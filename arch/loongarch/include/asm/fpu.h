...
#else
static inline void _init_fpu(unsigned int fcsr)
{
	(void)fcsr;
}

static inline void _save_fp(struct loongarch_fpu *fpu)
{
	(void)fpu;
}

static inline void _restore_fp(struct loongarch_fpu *fpu)
{
	(void)fpu;
}
#endif
...
#else
#define enable_fpu()		do { } while (0)

#define disable_fpu()		do { } while (0)
#endif

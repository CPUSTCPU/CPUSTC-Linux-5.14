...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
	u8 diagnostic_status;
	u8 diagnostic_state;
	u8 diagnostic_pos;
	u16 diagnostic_addr;
	u16 diagnostic_flags;
	bool diagnostic_timeout;
#endif
};
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
static void ocores_record_diagnostic(struct ocores_i2c *i2c, u8 stat)
{
	i2c->diagnostic_status = stat;
	i2c->diagnostic_state = i2c->state;
	i2c->diagnostic_pos = i2c->pos;
	i2c->diagnostic_addr = i2c->msg->addr;
	i2c->diagnostic_flags = i2c->msg->flags;
}
#else
static inline void ocores_record_diagnostic(struct ocores_i2c *i2c, u8 stat)
{
}
#endif
...
static void ocores_process(struct ocores_i2c *i2c, u8 stat)
{
	...
	if (stat & OCI2C_STAT_ARBLOST) {
		ocores_record_diagnostic(i2c, stat);
...
static void ocores_process_timeout(struct ocores_i2c *i2c)
{
	...
	spin_lock_irqsave(&i2c->process_lock, flags);
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
	ocores_record_diagnostic(i2c, oc_getreg(i2c, OCI2C_STATUS));
	i2c->diagnostic_timeout = true;
#endif
	i2c->state = STATE_ERROR;
	oc_setreg(i2c, OCI2C_CMD, OCI2C_CMD_STOP);
	spin_unlock_irqrestore(&i2c->process_lock, flags);
			    ...
			    bool polling)
{
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
	i2c->diagnostic_status = 0;
	i2c->diagnostic_state = STATE_START;
	i2c->diagnostic_pos = 0;
	i2c->diagnostic_addr = msgs->addr;
	i2c->diagnostic_flags = msgs->flags;
	i2c->diagnostic_timeout = false;
#endif
	...
	dev_err(i2c->adap.dev.parent,
		"SCCB diagnostic: addr=0x%02x direction=%s reason=%s phase=%s byte=%u status=0x%02x state=%u\n",
		i2c->diagnostic_addr,
		(i2c->diagnostic_flags & I2C_M_RD) ? "read" : "write",
		i2c->diagnostic_timeout ? "timeout" :
		(i2c->diagnostic_status & OCI2C_STAT_ARBLOST) ?
		"arbitration-lost" :
		(i2c->diagnostic_status & OCI2C_STAT_NACK) ? "NACK" :
		"unknown",
		i2c->diagnostic_state == STATE_START ? "address" :
		i2c->diagnostic_state == STATE_WRITE ? "data-write" :
		i2c->diagnostic_state == STATE_READ ? "data-read" : "unknown",
		i2c->diagnostic_pos, i2c->diagnostic_status,
		i2c->diagnostic_state);
#endif
	return error;
}

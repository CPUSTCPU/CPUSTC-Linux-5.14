...
static void cpustc_render_profile_bo(struct drm_device *drm,
				     unsigned int stage, u64 bo_id,
				     size_t size, u64 start_ns,
				     u32 handle, int ret)
{
	cpustc_render_profile_record(drm->dev, stage, bo_id, size,
				     ktime_get_ns() - start_ns, handle, ret);
}
#endif
...
#ifdef CONFIG_CPUSTC_RENDER_PROFILE
	cpustc_render_profile_bo(drm, CPUSTC_RENDER_BO_GEM_OBJECT, 0, size,
				 stage_start_ns, 0,
				 IS_ERR(cma_obj) ? PTR_ERR(cma_obj) : 0);
#endif
	if (IS_ERR(cma_obj))
		return cma_obj;
...
#ifdef CONFIG_CPUSTC_RENDER_PROFILE
	cpustc_render_profile_bo(drm, CPUSTC_RENDER_BO_GEM_DMA_ALLOC,
				 cma_obj->vaddr ? cma_obj->paddr : 0, size,
				 stage_start_ns, 0, cma_obj->vaddr ? 0 : -ENOMEM);
#endif
	if (!cma_obj->vaddr) {
...
#ifdef CONFIG_CPUSTC_RENDER_PROFILE
	cpustc_render_profile_bo(drm, CPUSTC_RENDER_BO_GEM_HANDLE,
				 cma_obj->paddr, size, stage_start_ns,
				 ret ? 0 : *handle, ret);
#endif
			    ...
			    struct drm_mode_create_dumb *args)
{
	...
	cma_obj = drm_gem_cma_create_with_handle(file_priv, drm, args->size,
						 &args->handle);
#ifdef CONFIG_CPUSTC_RENDER_PROFILE
	cpustc_render_profile_bo(drm, CPUSTC_RENDER_BO_DUMB_TOTAL,
				 IS_ERR(cma_obj) ? 0 : cma_obj->paddr, args->size,
				 total_start_ns,
				 IS_ERR(cma_obj) ? 0 : args->handle,
				 IS_ERR(cma_obj) ? PTR_ERR(cma_obj) : 0);
#endif
	return PTR_ERR_OR_ZERO(cma_obj);
}

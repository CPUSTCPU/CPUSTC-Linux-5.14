...
static int ov7670_write_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	while (vals->reg_num != 0xff || vals->value != 0xff) {
		...
		if (ret < 0) {
			...
			v4l_err(client,
				"diagnostic: register write failed reg=0x%02x value=0x%02x error=%d\n",
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
		v4l_err(client,
			"diagnostic: default register initialization failed error=%d\n",
			ret);
#endif
		return ret;
	}
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
		v4l_err(client,
			"diagnostic: manufacturer ID high read failed reg=0x%02x error=%d\n",
			REG_MIDH, ret);
#endif
		return ret;
	}
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
		v4l_err(client,
			"diagnostic: manufacturer ID low read failed reg=0x%02x error=%d\n",
			REG_MIDL, ret);
#endif
		return ret;
	}
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
		v4l_err(client,
			"diagnostic: product ID read failed reg=0x%02x error=%d\n",
			REG_PID, ret);
#endif
		return ret;
	}
...
#ifdef CONFIG_CPUSTC_CAMERA_DIAGNOSTICS
		v4l_err(client,
			"diagnostic: product version read failed reg=0x%02x error=%d\n",
			REG_VER, ret);
#endif
		return ret;
	}
